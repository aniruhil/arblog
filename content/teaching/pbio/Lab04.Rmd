...
What about the probability of more than two Tails? This should be just 1 minus the probability of two or fewer Tails. That is, 1 - 0.0545769, i.e., ```r 1 - p.2less ```, exactly the same result we got when we calculated <code>p.3plus</code>. Note that we saw this last week: $P(A^c) = 1 - P(A)$.
...
The best estimate is ```r p.sample ```
...
The standard error, ```r se.sample ```, measures the standard deviation of the sampling distribution of the sample proportion.
...
The 95% confidence interval is ```r ci.low; ci.high ```. This interval does not include the 14% cancer rate for the population, suggesting that cancer rate for the movie crew was not the same as that for the population. 
