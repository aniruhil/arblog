...
module.exports = function(eccent, q) {
  ...
  for (var i = 0; i < 30; i++) {
    ...
    cos_phi = Math.cos(phi);
    con = eccent * sin_phi;
    dphi = Math.pow(1 - con * con, 2) / (2 * cos_phi) * (q / (1 - eccent * eccent) - sin_phi / (1 - con * con) + 0.5 / ec {
    var def = arguments[1];
...
proj4.WGS84 = necent * Math.log((1 - con) / (1 + con)));
    phi += dphi;
    if (Math.abs(dphi) <= 0.0000000001) {
      return phi;
    }
...
exports.helmert = {
  a: 6378200.0,
  rf: 298.3,
  ellipseName: "Helmer    paramOutname = params[paramName];
...
exports.phi1z = function(eccent, qs) {
  ...
  var eccnts = eccent * eccentt 1906"
};
exports.hough = {
  a: 6378270.0,
  rf: 297.0,
...
exports.inverse = (proj.datumCode && proj.datumCode === 'none') {
    this.datum_type = PJD_NODATUM;
  }
...
/* Cassini forward equations--mapping lat,long to x,y
  ...
  /** Convert_Geocentric_To_Geodetic
   * The method used here is derived from 'An Improved Algorithm for
   * Geocentric to Geodetic Coordinat----------*/
exports.forward = function(p) {
...
exports.names = ["cea"];
...
},{"../common/adjust_lon":5,"../common/iqsfnz":13e Conversion', by Ralph Toms, Feb 1996
   */
  geocentric_to_geodetic_noniter: function(p) {
    var X = p.x;
    var Y = p.y;
  ...
  geocentric_from_wgs84: function(p) {
    ...
    if (this.datum_type === PJD_3PARAM) {
      ...
      var Dx_BF = this.datu,"../common/msfnz":15,"../common/qsfnz":20}],44:[function(require,module,exports){
var adjust_lon = require('../common/adjust_lon');
var adjust_lat = require('../common/adjust_lat');
exports.init = function() {
...
var EPSLN = 1.0e-10;
var qsfnz = require('../common/qsfnz');
var adjust_lon = require('../common/adjw proj4.Proj('WGS84');
proj4.Point = require('./Point');
proj4.toPoint = require("./common/toPoint");
proj4.defs = require('./defs');
proj4.transform = require('./transform');
  ...
  if (this.sphere) {
    ...
    case this.S_POLE:
      if (q >= 0) {
        ...
        y =     paramOutname = params[paramName];
      if (typeof paramOutname === 'function') {
        paramOutname(paramVal);
      }
      else {
...
exports.phi1z = function(eccent, qs) {
  ...
  var eccnts = eccent * eccentcoslam * ((this.mode === this.S_POLE) ? b : -b);
      }
      else {
        x = y = 0;
      }
...
exports.forward = function(p) {
  ...
  /* If the latitude is 90 deg, force the x coordinate to be "0 + false easting"
       this is done here because of precisio----------*/
exports.forward = function(p) {
...
},{"../common/adjust_lon":5,"../common/iqsfnz":13n problems with "cos(theta)"
       --------------------------------------------------------------------------*/
...
exports.forward = function(p) {
  ...
  for (n = 1; n <= 6; n++) {
    ...
    th_n_im1 = ,"../common/msfnz":15,"../common/qsfnz":20}],44:[function(require,module,exports){
var adjust_lon = require('../common/adjust_lon');
var adjust_lat = require('../common/adjust_lat');
exports.init = function() {
...
exports.inverse = function(p) {
  ...
  p.x /= Central point and azimuth method
...
exports.inverse = function(p) {
  ...
  if (Math.abs(up - 1) < EPSLN) {
    ...
    p.y = phi2z(this.e, ts);
    p.x = adjust_lon(this.long0 - Math.atan2(sp * Math.cos(this.gamma0) - vp * Math.sin(this.this.k0;
  p.y /= this.k0;
...
var EPSLN = 1.0e-10;
var qsfnz = require('../common/qsfnz');
var adjust_lon = require('../common/adjgamma0), Math.cos(this.bl * us / this.al)) / this.bl);
  }
  return p;
};
  ...
  if (this.sphere) {
    if (!this.m) {
      ...
      for (var i = MAX_ITER; i; --i) {
        ...
        lacoslam * ((this.mode === this.S_POLE) ? b : -b);
      }
      else {
        x = y = 0;
      }
...
exports.init = function() {
  ...
  if (this.lat_ts) {
    if (this.sphere) {
      this.k0 = Math.cos(t1 / this.alpha * (Math.log(Math.tan(Math.PI / 4 + b / 2)) - this.K) + this.e * Math.log(Math.tan(Math.PI / 4 + Math.asin(this.e * Math.sin(phy)) / 2));
    prevPhy = phy;
...
exports.forward = function(p) {
  ...
  /* If the latitude is 90 deg, force the x coordinate to be "0 + false easting"
       this is done here because of precisio= lat;
      return p;
    }
...
/**
    Transverse Mercator Forward  - lonn problems with "cos(theta)"
       --------------------------------------------------------------------------*/
  if (Math.PI / 2 - Math.abs(lat) < EPSLN) {
    delta_lon = 0;
  }
...
module.exports = function transform(source, dest, point) {
  ...
  if (source.datum && dest.datum && (checkNotWGS(source, dest) || checkNotWGS(dest, source))) {
    ...
    transCentral point and azimuth method
...
exports.inverse = function(p) {
  ...
  if (Math.abs(up - 1) < EPSLN) {
    ...
    p.y = phi2z(this.e, ts);
    p.x = adjust_lon(this.long0 - Math.atan2(sp * Math.cos(this.gamma0) - vp * Math.sin(this.form(source, wgs84, point);
    source = wgs84;
  }
  // DGR, 2010/11/12
  if (source.axis !== "enu") {
...
function cleanWKT(wkt) {
  ...
  if (wkt.GEOGCS) {
    ...
    //  wkt.from_gamma0), Math.cos(this.bl * us / this.al)) / this.bl);
  }
  return p;
};
...
exports.forward = function(p) {
  ...
  if (this.sphere) {
    if (Math.abs(lat) <= EPSLN) {
      ...
      x = this.a *greenwich=wkt.GEOGCS.PRIMEM.convert*D2R;
    //}
    if (wkt.GEOGCS.DATUM) {
 ...
 *     WGS84 ellips Math.sin(el) / Math.tan(lat);
      y = this.a * (adjust_lat(lat - this.lat0) + (1 - Math.cos(el)) / Math.tan(lat));
    }
  }
  else {
...
function LLtoUTM(ll) {
  ...
  var UTMNorthing = (k0 * (M + N * Math.tan(LatRad) * (A * A / 2 + (5 - T + t -= V;
        if (Math.abs(V) < EPSLN) {
          break;
        }
      }
...
exports.inverse = function(p) {
  ...
  while (Math.abs(phy - prevPhy) > 0.0000001) {
    ...
    //S = Math.log(Math.tan(Math.PI / 4 + phy / 2));
    S = 9 * C + 4 * C * C) * A * A * A * A / 24.0 + (61 - 58 * T + T * T + 600 * C - 330 * eccPrimeSquared) * A * A * A * A * A * A / 720.0)));
  if (Lat < 0.0) {
 ...
 *
 * @private
 * @param {number} lat The latitud1 / this.alpha * (Math.log(Math.tan(Math.PI / 4 + b / 2)) - this.K) + this.e * Math.log(Math.tan(Math.PI / 4 + Math.asin(this.e * Math.sin(phy)) / 2));
    prevPhy = phy;
    phy = 2 * Math.atan(Math.exp(S)) - Math.PI / 2;
  }
...
exports.inverse = function(p) {
  ...
  if (this.sphere) {
    ...
    if (rh <= EPSLN) {
      p.x = lon;
      p.y e in WGS84 to get the letter designator
 *     for.
...
/**
    Transverse Mercator Forward  - lon_ORIGIN_ROW_LETTERS.charCodeAt(index);
...
exports.names = ["Universal Transverse Mercator System", "utm"];
e first letter from a two-letter MGRS 100´k zone.
...
module.exports={
  "name": "proj4",
  "version": "2.3.15",
  "description": "Proj4js is a JavaScript library to transform point coordinates from one coordinate system to another, includi
},{"./tmerc":63}],65:[function(require,module,exports){
var adjust_lon = require('../common/adjust_lon');
var HALF_PI = Math.PI/2;
var EPSLN = 1.0e-10;
